...
`tools/list` at `https://gateway.pipeworx.io/codewars/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/codewars_user \
  -H 'Content-Type: application/json' \
  -d '{"username":"codewars_user"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/codewars_user`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
